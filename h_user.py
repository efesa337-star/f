...
async def _echo(message: Message, user: dict[str, Any], bot: Bot):
    ...
    # ПОЧИНЕНО: Если пользователь создает опрос (poll) прямо в боте,
    # мы автоматически создаем НОВЫЙ чистый опрос в твоем канале LOG_CHANNEL от имени канала!
    if message.poll is not None:
        try:
            # Извлекаем базовые параметры опроса (без correct_option_id и explanation для обычных регулярных опросов)
            ...
            # Оправляем чистый опрос без падений!
            published = await bot.send_poll(
            ...
            # Временно подменяем параметры сообщения на созданный в канале опрос
            message.chat.id = published.chat.id
            message.message_id = published.message_id
        except Exception as exc:
            log.warning("Не удалось опубликовать опрос в канал: %s", exc)
            await answer(message, "Ошибка: не удалось запустить опрос через канал.")
            return
